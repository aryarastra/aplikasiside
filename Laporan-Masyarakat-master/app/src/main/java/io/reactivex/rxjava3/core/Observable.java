...
public abstract class Observable<T> implements ObservableSource<T> {
    ...
    public static <T> Observable<T> concatArrayDelayError(ObservableSource<? extends T>... sources) {
        ...
        if (sources.length == 1) {
            Observable<T> source = wrap(sources[0]);
            return source;
        }
        Observable<T> source2 = fromArray(sources);
        return concatDelayError(source2);
    }
    ...
    public final void subscribe(Observer<? super T> observer) {
        ...
        try {
            ...
            Objects.requireNonNull(observer2, "The RxJavaPlugins.onSubscribe hook returned a null Observer. Please change the handler provided to RxJavaPlugins.setOnObservableSubscribe for invalid null returns. Further reading: https://github.com/ReactiveX/RxJava/wiki/Plugins");
    ...
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
    public final <R> R to(ObservableConverter<T, ? extends R> converter) {
    ...
    public final Flowable<T> toFlowable(BackpressureStrategy strategy) {
        ...
        switch (AnonymousClass1.$SwitchMap$io$reactivex$rxjava3$core$BackpressureStrategy[strategy.ordinal()]) {
            case 1:
    ...
    public final Stream<T> blockingStream(int capacityHint) {
        ...
        Disposable disposable = (Disposable) iterator;
        disposable.getClass();
